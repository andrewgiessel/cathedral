...
def test_shifted_chains_match_arviz_and_flag_nonconvergence():
    ...
    np.testing.assert_allclose(
        (result.rhat, result.ess_bulk, result.ess_tail, result.ess_mean, result.mcse_mean), expected, rtol=2e-12, atol=2e-12
    )
...
def test_optional_live_arviz_agreement(reference_case):
    ...
    result = diagnose_chains(draws)
    expected = (
        az.rhat(draws, method="rank"), az.ess(draws, method="bulk"), az.ess(draws, method="tail"),
        az.ess(draws, method="mean"), az.mcse(draws, method="mean"),
    )
    np.testing.assert_allclose(
        (result.rhat, result.ess_bulk, result.ess_tail, result.ess_mean, result.mcse_mean), expected, rtol=2e-12, atol=2e-12
    )
